...
import { $set } from 'packages/form-create/utils/modify';
import { deepCopy } from 'packages/form-create/utils/deepextend';
import is, { hasProperty } from 'packages/form-create/utils/type';
import extend from 'packages/form-create/utils/extend';
import { err, format } from 'packages/form-create/utils/console';
...
export default function Api(h) {
  ...
  const api = {
    ...
    append: (rule, after, child) => {
      ...
      let fields = Object.keys(h.fieldCtx),
        index = h.sort.length - 1,
        rules;

      if (rule.field && fields.indexOf(rule.field) > -1) {return err(`${rule.field} 字段已存在`, rule);}
