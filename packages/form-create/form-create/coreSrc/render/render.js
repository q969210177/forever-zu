...
import mergeProps from 'packages/form-create/utils/mergeprops';
import is, {hasProperty} from 'packages/form-create/utils/type';
import {_vue as Vue} from '../frame';
import {tip} from 'packages/form-create/utils/console';
import {invoke, mergeRule} from '../frame/util';
import toCase, {lower} from 'packages/form-create/utils/tocase';
...
export default function useRender(Render) {
  extend(Render.prototype, {
    ...
    makeVm(rule) {
      const vm = rule.vm;
      if (!vm) {return new Vue();} else if (is.Function(vm)) {return invoke(() => vm(this.$handle.getInjectData(rule)));} else if (!vm._isVue) {return new Vue(vm);}
      return vm;
    },
    mergeGlobal(ctx) {
